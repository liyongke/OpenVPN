...
### Issue H — Multiple devices on the same Wi-Fi were mislabeled as the same device

**Symptom:** Several active sessions from one public IP (same NAT/Wi-Fi) appeared with copied device labels.

**Root cause:** Real-IP-only hints can collide across different endpoint ports when several devices share one NAT egress IP.

**Resolution:**
- Prefer endpoint-aware hints (`ip:port`) generated by the `client-connect` hook.
- Portal inference now avoids applying real-IP-only hints when more than one active session shares the same public IP.
- Keep `real_endpoints` mappings populated by reconnecting clients after hook updates.
...
- Optional force-terminate action is available only when explicitly enabled (`PORTAL_CONTROL_ENABLED=1`) and token-gated.
- For targeted termination, set one of:
  - `PORTAL_OPENVPN_MANAGEMENT_TCP_SOCKET` / `PORTAL_OPENVPN_MANAGEMENT_UDP_SOCKET`
  - `PORTAL_CONTROL_TERMINATE_COMMAND` (custom command template)
